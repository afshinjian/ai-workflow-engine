# ORCH-006 — Workflow event and state v2

## Objective

Implement only the workflow event and state v2 capability described by Architecture v3.

## Reason it must occur at this point

Prerequisites are 005. Its contracts must be independently accepted before any dependent stage.

## Prerequisites

Every listed stage (005) must be REVIEW_APPROVED, with committed review evidence. The repository/state preflight in session-protocol.md must pass.

## Exact allowed files or directories

src/ai_workflow_engine/workflow/, cli.py, tests/test_workflow*.py, ORCH records.
...
workflow events/state/transitions/store/CLI.

## Public interfaces added or changed

attempt-scoped state show/record.
...
Events bind attempt/spec/candidate; integrity PASS; transition table exact.
...
Run independently and record exact argv, output digest and exit status: pytest -q tests/test_workflow*.py; pytest -q.
...
No decision/runner change.

## Rollback strategy

Revert v2 registrations; preserve old evidence.

## Independent review checklist

Exhaustive negative transitions and legacy read-only. The reviewer must differ from implementation/remediation actors and rerun every verification command from the committed implementation HEAD.

## Risks

Core state regression.
