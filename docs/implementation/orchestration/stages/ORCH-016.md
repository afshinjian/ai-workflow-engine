# ORCH-016 — Approval v2 and Ed25519

## Objective

Implement only the approval v2 and ed25519 capability described by Architecture v3.
...
src/ai_workflow_engine/approval/, dependency metadata, tests/test_approval_v2.py, ORCH migration/records.

## Files expected to be created

models/canonicalizer/verifier/vectors.
...
pure verify; approval verify no-consume.

## Schemas added or changed

ApprovalEnvelope 2.0.0; TrustKeyRecord 1.0.0.
...
RFC8785; non-circular ID; detached signature; exact fields; no private key.
...
Run independently and record exact argv, output digest and exit status: pytest -q tests/test_approval_v2.py tests/test_approval.py; pytest -q.
...
No key storage/signing/replay.

## Rollback strategy

Remove v2/dependency/adapter.

## Independent review checklist

Crypto/canonicalization and dependency provenance. The reviewer must differ from implementation/remediation actors and rerun every verification command from the committed implementation HEAD.

## Risks

Cross-language canonical mismatch.
