...
def test_load_rejects_duplicate_json_keys(engine_config: EngineConfig) -> None:
    ...
    duplicated = body_without_closing_brace + ',"schema_version":"1.2"}\n'
...
def test_load_rejects_legacy_1_1_schema_sidecar(engine_config: EngineConfig) -> None:
    # After the schema bump (T-307, 1.1 -> 1.2), a stored 1.1 sidecar must be rejected on load.
    rendered = _rendered(engine_config)
    paths = save(rendered)
    text = paths.metadata.read_text(encoding="utf-8")
